...
@version	3.2.3
@dated		27/12/2018
@notes		- Fixed another critical error with the remove comments functionality that caused some major bugs.
			- Fixed a critical error with the remove comments functionality in Javascript and CSS that caused some major bugs. 
			- Rewrote the remove_comments() function in HTML minifier to make it more efficient. The old function incorrectly identified regexes and strings previously, and was not efficient.
			...
			- Added a combine_rsc function for combining multiple CSS / JS files.
			----------------
			- Fixed a bug with the 'ignore_async_and_defer_tags' option. They behaved opposite of what they should have.
...
class HTMLMinifier {
	...
	// Takes an array of relative paths in $files_array of $filetype.
	public static function combine_rsc($files_array,$filetype,$options,$webrootDir = false,$cache_key = '') {
		...
		// No cached result. Start computing the result.
		$filetype = strtolower($filetype);
		switch($filetype) {
		case 'js': case 'javascript':
			if(!empty($options['combine_use_strict'])) $out = '"use strict";';
			else $out = '';
			break;
		default:
			$out = '';
		...
		if($webrootDir) $webrootDir = rtrim($webrootDir,'/\\');
		
		$file_sep = PHP_EOL; // Separator for files.
		if(!empty($options['show_signature'])) $show_sig = $options['show_signature'];
		else $show_sig = self::$Defaults['show_signature'];
		$options['show_signature'] = false;
		
		$orig_size = 0; // Record of file sizes of all files.
		$request_protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off') || $_SERVER['SERVER_PORT'] == 443 ? 'https' : 'http';
		
		foreach($files_array as $f) {
						
			// Relative URLs are unsupported. Only absolute URLs.
			if(preg_match('@^(?:https?://|//|[\\\\/])@',$f,$match)) {
				
				// If address starts with //, make it a http address.
				if(strpos($match[0],'//') === 0) $f = $request_protocol . ':' . $f;
			
				// If there is a query string in the address, make a HTTP(s) request.
				$pathinfo = pathinfo($f);
				if($match[0] == '/' && isset($pathinfo['extension']) && strrpos($pathinfo['extension'],'?') !== false) {
					$f = base_url(substr($f,1));
					$match[0] = $request_protocol . '://';
				}
			
				// If this is a HTTP(s) request and we are unable to use file_get_contents on it, use cURL.
				if(strpos($match[0],'http') === 0 && (!$allow_url_fopen || !$webrootDir)) {

					$ch = curl_init();
					if(!$ch) trigger_error('Unable to use cURL. The module may not be enabled in your PHP installation.',E_USER_ERROR);
					curl_setopt($ch, CURLOPT_URL, $f);
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
					curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
					$data = curl_exec($ch);
					if($data === false) trigger_error(sprintf('Invalid response from URL %s.',$match[0]),E_USER_ERROR);

				} else {
				
					// Get the data using file_get_contents().
					if($webrootDir && preg_match('@^[\\\\/]$@',$match[0])) {
						$data = file_get_contents($webrootDir . $f);
					} else $data = file_get_contents($f);
					
				}
				
				// Replace all relative URLs in CSS with absolute ones.
				switch($filetype) {
				case 'css':
					if(preg_match_all('@url\\(([\\s\\S]*?)\\)@i',$data,$urls)) {
						foreach($urls[1] as $key => $url) {
							$url = preg_replace('@^https?://@i','//',trim(trim(trim($url),'\'"')));
							if(preg_match('@^[^\\\\/]@',$url,$ups)) {
								if(!preg_match('@^data\\:@i',$url))
									$url = $pathinfo['dirname'] . '/' . $url;
							}
							$data = self::replace($urls[1][$key],$url,$data);
						}
					}
					break;
				case 'js': case 'javascript':
					//$data = preg_replace('@^(?:\'use strict\'\\s*;|"use strict"\\s*;)@','',trim($data),1);
					break;
				}
				
				$orig_size += strlen($data);
				$out .= self::minify_rsc($data,$filetype,$options) . $file_sep;
				continue;
			}
		}
	...
	private static function process_script_options($source,$options,&$comments,&$conditionals,&$wrapped_conditionals) {
		...
		if($options['shift_script_tags_to_bottom']) {
			...
			if(!empty($script_combine)) $appendix .= '<script>'.$script_combine.'</script>';
	...
	private static function remove_comments($source,$type = 'javascript', $ignoreCdataComments = true) {
		...
		// Encapsulated content that we want to capture (in regex without delimiters).
		$encap_start = array(
			's_quote' => "'", 
			'd_quote' => '"'
		);
		...
		if($type === 'css') {
			$encap_start['url_func'] = '(?:url|image)\\(';
			$encap_end['url_func'] = '\\)';
		} else {
			$encap_start['regex'] = '(?<=^|[[{};:=?(,*!]|&&|\\|\\||return)\s*/(?![/\\*])';
			$encap_end['regex'] = '(?:(?<!\\\\)|(?<=^|[^\\\\])(\\\\{2})+)/[gim]?';
		}
		
		// Comments that we want to capture.
		$comments_start = array(
			's_comment' => '//',
			'm_comment' => '/\\*'
		);
		$comments_end = array(
		...
		$total_start = array_merge($encap_start,$comments_start);
		$total_end = array_merge($encap_end,$comments_end);
		
		// Create regex and start capturing.
		$start_regex = '@(?:' . implode('|',$total_start). ')@i';
		
		// Start capturing.
		$offset = 0;
		$comments_store = array();
		while(preg_match($start_regex,$source,$match,PREG_OFFSET_CAPTURE,$offset)) {
						
			// Find the item that has triggered encapsulation.
			$idx = false;
			foreach($total_start as $k => $e) {
				if(preg_match('@'.$e.'@i',$match[0][0])) {
					$idx = $k;
					break;
				}
			}
			
			// Find the corresponding end encapsulator.
			$offset = $match[0][1];
			if(preg_match('@' . $total_end[$idx] . '@i',$source,$match_end,PREG_OFFSET_CAPTURE,$offset + strlen($match[0][0]))) {
				$en_len = strlen($match_end[0][0]);
				$captured_str = substr($source,$offset,$match_end[0][1] - $offset + $en_len);
				
				// If this is a regex, grab any open square brackets inside the captured string, as we want to exclude using contents within to end the regex.
				if($idx === 'regex') {
					while(preg_match_all('@(?:(?<!\\\\)|(?<=^|[^\\\\])(?:\\\\{2})+)\\[([\\s\\S]*?(?:(?<!\\\\)|(?<=[^\\\\])(?:\\\\{2})+)\\])?@',$captured_str,$regex_match,PREG_OFFSET_CAPTURE)) {
						if(empty($regex_match[1][count($regex_match[1])-1])) {
							if(preg_match('@' . $total_end[$idx] . '@i',$source,$match_end,PREG_OFFSET_CAPTURE,$offset + strlen($captured_str))) {
								$captured_str = substr($source,$offset,$match_end[0][1] - $offset + $en_len);
							} else {
								// ERROR MESSAGE, end encapsulation tag not found.
								$ftype = $type === 'css' ? 'CSS' : 'Javascript';
								trigger_error(sprintf('There is an error in your %s file. An encapsulator like a string or regex was opened and was not closed: %s.',$ftype,$captured_str),E_USER_ERROR);
							}
						} else break;
					}
				}
				
				//var_dump($captured_str);
				// If this is a comment, mark it for removal.
				if(array_search($total_start[$idx],$comments_start) !== false) {
					array_push($comments_store,$captured_str);
				}
				
				$offset = $match_end[0][1] + $en_len;
			} else {
				// ERROR MESSAGE, end encapsulation tag not found.
				$ftype = $type === 'css' ? 'CSS' : 'Javascript';
				trigger_error(sprintf('There is an error in your %s file. An encapsulator like a string or regex was opened and was not closed: %s.',$ftype,$captured_str),E_USER_ERROR);
		...
		// Remove all comments inside the source.
		foreach($comments_store as $k => $e) $source = self::replace($e,'',$source);
